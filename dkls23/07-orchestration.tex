...
\begin{align}
  ...
  \lambda(j,\mathbb{S}) &:= \prod_{k\in\mathbb{S}\setminus\{j\}} \frac{-k}{j - k}, \\
  \label{eq:orch:lagrange-interp}\tag{interp}
  Y &\phantom{:}\stackrel{?}{=} \sum_{j\in\mathbb{S}}\lambda(j,\mathbb{S}) \cdot S_j.
\end{align}
...
【检查-P】重新计算 $\mathrm{Com}_{R,j}$，检查与 Round 1 所收到的是否相等。
...
在发送签名份额之前，将该 PreSignature 标记为已使用，然后交换 $s_{0,i}, s_{1,i}$。
