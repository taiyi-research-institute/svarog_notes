...
$\bigl((\beta_{\ell,i,j},\rho^{\beta_{\ell,i,j}}_{\ell,i,j}) \mid \ell\in[hk,(h+1)k)\bigr)$，
...
\begin{equation}
  ...
  d := \mathrm{Hash}
  \circ \mathrm{RO}\bigl(C,(\mathrm{Com}_{R,j} \mid j\in\mathrm{sort}(\mathbb S))\bigr).
\end{equation}
\end{patchnote}
...
$\bigl(\rho^{\beta_{\ell,j,i}}_{\ell,j,i} \mid \ell\in[0,L)\bigr)$，
