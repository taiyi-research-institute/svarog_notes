...
后续笔记的路线是 "OT 扩展": 椭圆曲线只在 keygen 阶段跑 $\kappa=256$ 次 base OT ([Endemic OT](./03-endemic-ot.md)), 生成可长期保存的种子 ([PPRF 与 GGM 树](./04-pprf.md)); 签名阶段吃这些种子, 用哈希/异或等对称运算扩展出任意多的 OT 实例 ([IKNP03 OT 扩展](./01-iknp03.md), [SoftSpokenOT](./05-softspoken.md)). 对称运算比椭圆曲线便宜若干数量级, 详见 [IKNP03 OT 扩展](./01-iknp03.md) 的小结 (密钥交换 "基于异或运算的消去律" 而非椭圆曲线), 以及 [随机 VOLE](./06-rvole.md) 末尾的讨论 "Keygen 真正摊销的是什么".
...
当 $d_{t^*}=0$ 时下游签名照常成功, 当 $d_{t^*}=1$ 时签名失败. Sender 观察成败, 就白得了 $x_b$ 的一个比特, 代价只是一场失败的会话. 这叫 selective failure 攻击. 多场会话反复试探的后果, 详见 [KOS15 恶意安全](./02-kos15.md) 的 "追问2".

后续笔记的一致性检查正是冲着这类攻击去的: [随机 VOLE](./06-rvole.md) 的校验负载抓在不同 OT 实例上使用不一致 $w$ 的 Sender, 与上述攻击同型; [KOS15 恶意安全](./02-kos15.md) 的挑战-响应则抓 OT 扩展里篡改 $u$ 矩阵的 Receiver. 顺带一提, 就连 base OT 本身, DKLs23 也不用本章的朴素二选一 OT, 而是用安全定义更清晰的 Endemic OT ([Endemic OT](./03-endemic-ot.md)).
