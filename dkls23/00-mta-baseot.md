...
* 双方半诚实, 即都按剧本出牌. 恶意方能造成什么破坏, 见末章 "朴素在哪".
* 依赖子协议: 每个比特跑一次二选一 OT (下一节), 共 $\ell$ 次, 假设其安全承诺成立. OT 藏住各比特 $d_t$, 就是藏住 $x_b$; OT 藏住没选中的那条消息, $x_a$ 就被现摇的 $r_t$ 一次一密地掩盖着.
