...
\begin{patchnote}{2026-976｜完全版改用随机内核，再转换指定输入}
【初版笔记】完全版将 Sender 持有的指定输入 $w_i$ 直接放入修正矩阵。

【补丁写法】
2026/976 的内核先生成双方输入均随机的 VOLE，再用标量偏移适配 Sender 指定输入。
本节先描述该随机内核，令 $w_i$ 表示新鲜随机的功能负载，最后给出指定输入转换。
...
【补丁｜2026-976】$N_2$ 对应原文的参数 $\rho$；本篇采用 2026/976 的条件，在 secp256k1 下取 $N_2=1$。
...
【初版笔记】$L=\kappa+2\lambda_s$，$N_2=\lceil\kappa/\lambda_c\rceil$。
...
【初版笔记】gadget 采用可信公共随机串，可跨实例复用；挑战只绑定会话号、用途与 $\tilde a$。

【补丁写法】
【公开随机量】默认 Variant III 在协议开始时派生公开 gadget：
\begin{equation}\label{eq:rvole:gadget-iii}
\boldsymbol g:=\mathrm{RO}_{\mathbb Z_n^L}
  \bigl(\mathrm{Enc}(\mathtt{sid},\texttt{"gadget-III"})\bigr).
\end{equation}
这里按 2026/976 附录 B.3 的会话随机预言机设置执行，域采样遵循上述拒绝采样规则。
每个有向 RVOLE 实例使用唯一的 $\mathtt{sid}$；本篇不再默认跨实例复用同一 gadget。
...
挑战的 OT 绑定按下段补充，不能只照抄原文 Protocol 5.2 的哈希输入。

【OT 接口与 transcript】2026/976 的定理使用 Sender Random OT：
即使 Sender 恶意，理想功能也为其生成随机消息，禁止其任意指定两侧 OT 输入。
Endemic OT 的抽象允许恶意 Sender 自选输入，不能直接替代这个更强的接口。
具体的 Base OT、PPRF、SoftSpoken 组合必须说明最终随机 OT 接口的安全保证；
本篇记录组合所需条件，不把现有笔记中的种子扩展自动视为完成了该证明。

记 $T_{\mathrm{OT}}$ 为双方按固定顺序编码的具体 OT 通信记录，或其带域分离摘要。
它包含产生本批 OT 的初始化记录与本次扩展消息，并绑定双方身份、方向、协议版本和批次号。
采用已有初始化时，应保存其摘要并与本次扩展记录一起绑定；不得拼入双方不同的秘密种子或输出。
在挑战中加入 $T_{\mathrm{OT}}$ 是 2026/976 第 4.5 节建议的实施措施。
若以它修补 Endemic OT 路线，还须证明具体 transcript 绑定 Sender 的 OT 输入；
仅在哈希输入里增加一个字段，不能替代这项性质。
\end{patchnote}
...
令 $M_2:=\mathrm{Enc}(\tilde a,\eta,\mu)$，再派生
\begin{equation}\label{eq:rvole:gadget-ii}
\boldsymbol g:=\mathrm{RO}_{\mathbb Z_n^L}\bigl(
  \mathrm{Enc}(\mathtt{sid},\texttt{"gadget-II"},T_{\mathrm{OT}},M_2)\bigr).
\end{equation}
...
【初版笔记】$(\theta_k)_k=\mathrm{RO}(\mathrm{Enc}(\mathtt{sid},\texttt{"challenge"},\tilde a))$。
...
\mathrm{Enc}(\mathtt{sid},\texttt{"challenge"},T_{\mathrm{OT}},\tilde a)\bigr).
\\
...
\qquad i\in[0,N_1).
\end{equation}
Receiver 在内核校验通过并获得 $\beta$ 后，把输出修正为
...
Variant II 的 gadget 仍从内核消息 $M_2$ 派生，指定输入偏移属于外层转换。
两方独立预处理则只保存随机内核输出，直到在线阶段才发送偏移；不要把私钥提前放入随机内核。
\end{patchnote}
...
【补丁写法】
在 OT 输出已经固定的前提下，Sender 在随机内核发一条消息 $(\tilde a,\eta,\mu)$。
以下只复盘该内核的一致性检查，指定输入偏移按上一节另行转换。
