...
DKLS23 使用随机乘法因子 $\beta$，再由 ECDSA 编排层将其转换为所需的乘法关系。
若直接以长期秘密的比特作为 OT 选择位，恶意 Sender 可以让检验结果依赖某些选择位，
并借助外层可观察的成功或失败结果探测这些比特，这称为 selective failure。
本协议每次使用新鲜随机的 $\vec\beta$，避免反复探测同一条选择串；
但新鲜性本身不能证明聚合值 $\beta$ 在泄露后仍接近均匀，也不能替代外层的失败处理。
参见\href{https://eprint.iacr.org/2023/765.pdf}{DKLS23 Protocol 5.2、\S6.1 及 Protocol 3.6 第 8～9 步}。

这里需区分三个对象：OT 选择串 $\vec\beta$、聚合标量 $\beta$，以及攻击造成的通过或失败事件。
$L=\kappa+2\lambda_s$ 为随机子集和聚合提供统计安全余量，不能理解为允许任意泄露 $2\lambda_s$ 比特。
原文同时控制攻击的通过概率与通过条件下的分布偏差，再证明真实执行可由理想功能模拟。
下面的\hyperref[sec:rvole:selective-failure]{selective failure 安全边界}集中列出结论与文献位置。
...
参见\href{https://eprint.iacr.org/2023/765.pdf}{Protocol 5.2 第 3～4 步及 Claim 6.7}。
...
【安全模型】以下引用的定理针对理想向量 OT 扩展和不可编程全局随机预言机模型，
允许静态腐化至多一方的恶意多项式时间攻击者。
实际使用 SoftSpoken 与短种子扩展时，以其满足所需向量 OT 扩展接口的计算安全性为前提；
仅仅把一个函数命名为 PRF 或 Hash，不能替代这一前提。
因此，本文的隐私承诺是相对于理想功能及组合假设的计算不可区分性，允许中止及可忽略误差，
而不是对任意哈希实现的「零泄露」断言。
依据见\href{https://eprint.iacr.org/2023/765.pdf}{\S5.1、Theorem 6.1、Lemmas 6.2 与 6.11}。
...
【安全承诺】在上述模型与组合假设下，Receiver 获得的视图不泄露超出理想功能所允许的信息。
对恶意 Sender，安全目标是实现随机 VOLE 理想功能，允许中止；不承诺原始选择串绝不泄露。
selective failure 的概率与泄露边界见\hyperref[sec:rvole:selective-failure]{下文专节}。
逐条核对见 2.3 节。
...
\subsection{核对安全承诺}
\subsubsection{\texorpdfstring{$w_i$ 对 Receiver 的隐私}{w 对 Receiver 的隐私}}
...
\subsubsection{selective failure 的安全边界}
\label{sec:rvole:selective-failure}

【攻击能获知什么】恶意 Sender 可以构造错误消息，使检验是否通过依赖他猜测的选择位。
如果外层暴露检验结果，一次针对单个位的探测便可能确定该位；
若同时猜测多个位，通过意味着这些猜测全部命中，失败通常只说明至少一个猜测不对。
因此，不能把一个成功或失败标志解释为「每次执行最多泄露一个指定比特」，
也不能把失败解释为逐项返回所有被猜错的位置。
原文对这类攻击的刻画见\href{https://eprint.iacr.org/2023/765.pdf}{\S6.1，Hybrid $H_3$ 与 Claims 6.4、6.8}。

【通过概率与剩余熵】沿用原文，令 $\Delta$ 表示校验偏差可能使检验结果依赖选择位的位置集合，
记 $d=|\Delta|$；它是证明中定义的集合，不是协议输出的泄露位置列表。
在原文 $H_5$ 的条件下，Claim 6.8 给出通过概率至多 $2^{-d}$。
对于 $d\le2\lambda_s$ 且两实验均通过的情形，Claim 6.10 对 $H_5$ 与 $H_6$ 给出条件统计距离上界
\begin{equation}
  2^{-(L-\kappa-d)/2}+LQ\,2^{-\lambda_c},
\end{equation}
其中 $Q$ 是攻击者的随机预言机查询次数，本文 $L$ 对应原文 $\xi$。
第一项随 $d$ 增大而变差；当 $d=2\lambda_s$ 时，该项已为 1，不能据此声称通过后仍接近均匀。
这两个界属于原文的混合实验，不能省略前提后当成任意实现、任意外部泄露的通用保证。
参见 \href{https://eprint.iacr.org/2023/765.pdf}{DKLS23 Claims 6.7～6.10，正文第 50～52 页}。

【最终保证】证明把通过概率、通过时的条件距离及失败分支合起来，控制完整执行的区分优势。
结论允许罕见的通过事件携带较强信息，并不要求每个条件分布都保持同样的统计安全参数。
上述子集和估计要求原文规定的公开随机 gadget 分布，不能仅从「向量比标量长」推出。
其统计工具是 Impagliazzo 与 Naor 的子集和估计，即 [IN96] Proposition 1.1 第 2 部分；
具体应用及两分支的合并见
\href{https://eprint.iacr.org/2023/765.pdf}{DKLS23 Claim 6.5、Claim 6.10 与 Hybrid $H_6$ 末尾}。
[IN96] 为 Russell Impagliazzo、Moni Naor，
《Efficient Cryptographic Schemes Provably as Secure as Subset Sum》，
Journal of Cryptology，9（4）：199～216，1996。

【外层与多会话】不能假定 Sender 的完整视图只多出一个 abort 比特。
Protocol 5.2 在采样阶段就把聚合标量交给环境；证明因此必须处理环境已获知 $\beta$ 的联合视图，
包括它与检验结果、输出份额的相关性，而不是继续把 $\beta$ 当作对环境隐藏的秘密。
安全结论见 \href{https://eprint.iacr.org/2023/765.pdf}{Lemma 6.2、Theorem 6.1}：
在理想 OT 扩展及不可编程全局随机预言机模型下，对静态腐化的恶意多项式时间攻击者实现 UC 安全。
本文的参数对应与实际实例化前提已在\hyperref[sec:rvole:parameters-model]{参数、编码与安全模型}中列明。
每次重新采样选择串只能避免直接复用该串；ECDSA 的长期密钥安全还依赖外层协议及其失败处理，
尤其是 \href{https://eprint.iacr.org/2023/765.pdf}{Protocol 3.6 第 8～9 步}规定的关联会话终止与后续拒绝交互。
...
原文分别处理偏差被随机挑战掩盖，以及偏差使检验依赖选择位这两种情形。
下面只说明攻击机会；完整的模拟论证见\href{https://eprint.iacr.org/2023/765.pdf}{DKLS23 \S6.1}。

\begin{itemize}
  \item 【研磨（grinding）】Sender 反复重选 $\tilde a$，寻找能掩盖偏差的挑战。
  攻击机会随随机预言机查询次数增长；原文 Claim 6.7 对单个偏差位置给出 $Q\,2^{-\lambda_c}$ 的界。
  这依赖原文的挑战分布与校验参数，不能仅凭「使用了哈希」断言研磨不可行。
  \item 【选择性失败（selective failure）】即使偏差没有被挑战掩盖，Sender 仍可能猜中有关选择位而通过。
  猜错导致检验失败；多个位置只产生整体检验结果，不逐项报告。
  较长的随机选择串为子集和估计提供余量，却不保证任意条件泄露后聚合值仍均匀。
  通过概率与条件距离的区别见\hyperref[sec:rvole:selective-failure]{selective failure 的安全边界}，
  对应原文 Claims 6.8～6.10。
\end{itemize}
