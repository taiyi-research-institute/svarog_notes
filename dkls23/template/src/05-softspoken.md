...
PPRF 与 GGM 树 把$k$个二选一 Base OT 合成一棵$q=2^k$叶的打孔树. 本篇是 IKNP03 OT 扩展 末节 "半诚实的边界" 第 (3) 条路线的正主: SoftSpokenOT 用$\kappa/k$棵打孔树顶替 IKNP 的$\kappa$行种子, 把半密钥$u$的行数从$\kappa$压到$\kappa/k$, 拿计算量换带宽. 一致性检查沿用 KOS15 恶意安全 的骨架: 承诺-挑战-响应, 再用 Fiat-Shamir 变换压成单条消息. 产出的随机 OT 密钥由 随机 VOLE 兑换成 MtA.
...
ECDSA Keygen 阶段跑$\kappa = 256$个 Base Endemic OT 实例 (Endemic OT). 对每个实例$i\in 0, \kappa)$:

* Sender 得到两侧串$\left(\rho_i^0, \rho_i^1\right)$, 其中$\rho_i^b \in \left\{0,1\right\}^\lambda$.
* Receiver 得到选择位$\beta_i$与单侧串$\rho_i^{\beta_i}$.

 正文将把$\rho$重载为 Extended OT 的输出密钥 (与下游 [随机 VOLE 对齐). 类似地, 正文将把$\beta$重载为 Extended OT 的选项.

## 1.2. PPRF 的接口规格

发生于 ECDSA Keygen 阶段, 紧跟 Base OT: 建好的树随 keyshare 持久落库, 供日后每次 Sign 反复取用.

每$k = 4$个 Base (Endemic) OT 实例喂给 PPRF 与 GGM 树 合成一棵 GGM 树, 共$\kappa/k = 64$棵. 对于树编号$i \in 0, \kappa/k)$:

* 叶子共$q = 2^k = 16$片, 记为$\mathcal{T}_{i,x} \in \left\{0,1\right\}^\lambda$,$x \in [0, q)$. 本篇笔记只需要叶子层, 故略去 [PPRF 与 GGM 树 的层上标, 把树编号挪进下标.
* PPRF Sender 持全部$q$片叶子.
* PPRF Receiver 持$q-1$片叶子, 缺打孔叶子$\mathcal{T}_{i,\delta_i}$. 打孔下标$\delta_i \in 0, q)$即 [PPRF 与 GGM 树 的$\hat y$, 由他的 Base OT 选择位决定, 只有他知道.
...
Extended OT 是随机 OT: 两侧协商出随机密钥串, 真正的 payload 传输推迟到 [随机 VOLE 完成.
...
|$\lambda$| 叶子内容长度 (数值同$\kappa$, 见 PPRF 与 GGM 树) |    256    |
...
* 双方恶意. Receiver 掺假由挑战-响应防御. Fiat-Shamir 使得 Sender 无法指定挑战. Sender 的另一类作恶 (selective failure) 是另一条线, 见 随机 VOLE.
* PPRF 来历合规: PPRF 与 GGM 树 的安全承诺未被破坏, 即: 打孔叶内容对本篇 Sender 均匀随机, 打孔位置对本篇 Receiver 保密.
*$\mathtt{PRG}$,$\mathtt{XOF}$,$\mathtt{Hash}$视作随机谕言机.

通信形态: 全程只有一条 Receiver$\to$Sender 的消息. 原论文是三轮 Sigma 形 (Receiver 承诺, Sender 挑战, Receiver 响应), Fiat-Shamir 后压成一条, 同 3.2. 实施.
...
$$
r_{i,x} := \mathtt{PRG}\left(\mathtt{sid},\, i,\, \mathtt{tag4},\, \mathcal{T}_{i,x}\right) \in \left\{0,1\right\}^{L'}. \tag{leaf}
$$

 域分离参数:$\mathtt{tag4}$续接 [PPRF 与 GGM 树 的$\mathtt{tag1}$~$\mathtt{tag3}$, 把叶子扩张与其他事由隔离开; 树编号$i$顶替那篇的$\mathtt{tid}$.
...
陪跑的作用详见 3.2.(0) Receiver 延长选择向量.

生成半密钥兼选项承诺$u$.

$$
u_{i,*} := \hat\beta \oplus \sum_{x \in 0, q)} r_{i,x}. \tag{umat}
$$
...
Receiver 此刻就能算, Sender 收到$u$后也能算. 取$S = 128$是为了让$\mathbb{GF}(2^{128})$的乘法享受 AES 相关的硬件指令. 域运算详见 [杂记：F_2^k.

Fiat-Shamir 的收益不只是省一轮通信, 更在于:$\chi$被随机谕言机固定住, 双方都无法指定挑战. 详见 3. 进阶版 KOS15.
...
\right\} 
~ \oplus ~
\mathrm{bit}_b(\delta_i) \cdot u_{i,*}. \tag{wmat}
$$
...
$$
w_{i',*} = v_{i',*} \oplus \mathrm{bit}_b(\delta_i) \cdot\hat\beta. \tag{wv-row-eq}
$$
...
$$
\Delta := \delta_0 \,\|\, \delta_1 \,\|\, \cdots \,\|\, \delta_{\kappa/k - 1} \in \left\{0,1\right\}^{\kappa}. \tag{delta}
$$
...
\stackrel{?}{=}
\tau_{i'}\oplus\Delta_{i'}\cdot\tilde\beta. \tag{verify}
$$
...
掺假必抓 (概率意义): Receiver 若在不同的行使用不同的选项串, 通过的概率与 [B. 详细论证 Sender (verify) 原理类似. 检查的可靠性由 Schwartz-Zippel 引理与$S = 128$比特的域尺寸兜底, 不再赘述.
...
Hash 这一下不能省: 它把 "两个原像差一个公差" 升格为 "两把密钥各自独立" ([2.3.2. 与 MtA 与 Base OT 对比思路), 并掺入域分离参数$(\mathtt{sid}, \mathtt{tag5}, j)$. 若不 Hash, 泄露任何一对密钥就等于泄露$\Delta$.

# 3. 总结

### 3.1. 核对安全承诺

**"Sender 不知道$\beta$".** (umat) 的掩码$\sum_x r_{i,x}$含打孔叶的扩张$r_{i,\delta_i}$, Sender 不知道它, 掩码对他均匀 —— 一次一密. 归约链: 窥$\hat\beta$$\to$求$r_{i,\delta_i}$$\to$得打孔叶$\mathcal{T}_{i,\delta_i}$, 破 PPRF 与 GGM 树 的承诺 "打孔叶学不到", 或破$\mathtt{PRG}$. 新增消息$\tau, \tilde\beta$各被一段新鲜陪跑 ($v^\mathrm{ext}$/$\beta^\mathrm{ext}$) 一次一密掩护, 对应 KOS15 恶意安全 §2.3.2 机制一的修补; 挑战被 Fiat-Shamir 钉死, Sender 没有定向挑$\chi$的机会, 对应机制二的修补.

**"Receiver 不知道$\rho^{1-\beta_j}_j$".** 归约到 "算不出另一个原像", 归约到 "不知道公差$\Delta$".$\Delta$由打孔下标拼成, 归约到 PPRF 与 GGM 树 的承诺 "打孔位置学不到", 落脚在 Base OT 藏住选择位. 恶意 Receiver 想靠掺假探$\Delta$, 由 (verify) 兜住: 掺假以压倒性概率败露, 即便侥幸也只换得少数比特,$\Delta$剩余熵仍然巨大, 论证与 KOS15 恶意安全 §2.3.1 同型.

**"一致性".** 诚实双方由 (wv-eq) 自动对齐. Receiver 掺假由 (verify) 抓; Sender 在本协议零下行消息, 没有作恶的载体 —— 他的 selective failure 藏在下游拿密钥加密的时刻, 见 随机 VOLE.
