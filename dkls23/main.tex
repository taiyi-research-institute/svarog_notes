...
% 数学正文使用 Latin Modern Math；黑板粗体使用经典的有衬线 AMS 字形。
% unicode-math 必须最后加载 (amsmath/amssymb 之后)。
\usepackage{amsmath,amssymb,mathtools}
\usepackage{unicode-math}
\setmathfont{latinmodern-math.otf}
% 粗体数学版本 (\boldsymbol / \mathversion{bold} 依赖): Latin Modern Math 无原生粗体,
% 改用同源 (Computer Modern 一脉) 的 NewCM Math Bold; 缺此行则 \boldsymbol 静默失效 (不报错)。
...
% 补丁保留在相应笔记中，用黑体标记和上下横线界定范围，正文可以跨页。
\newenvironment{patchnote}[1]{%
  \par\noindent{\heitiLatin\heiti 【补丁】#1}\par\nopagebreak
  \noindent\rule{\linewidth}{0.4pt}\par\nopagebreak
}{%
  \par\nopagebreak\noindent\rule{\linewidth}{0.4pt}\par\nopagebreak
  \noindent{\heitiLatin\heiti 【补丁结束】}\par
}
