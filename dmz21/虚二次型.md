教材 Binary Quadratic Forms An Algorithmic Approach 的阅读笔记. 仅用于查阅定义定理, 不建议一上来就读.

Head of Chap 1. A binary form $f=(a,b,c)$ is defined as
$$
f(x,y):=ax^2+bxy+cy^2,
$$
such that $a^2+b^2+c^2 > 0$, i.e. not all coefficients are zero.
...
* $\mathbf{S}^2=\begin{bmatrix}-1&0\\0&-1\end{bmatrix}=-\mathbf{I}_2$,
* $\mathbf{S}^3=\begin{bmatrix}0&1\\-1&0\end{bmatrix}=-S$,
