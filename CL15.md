...
输入是一个理想类 $\left[\mathfrak{a}\right] \in C(\mathcal{O}_{\Delta_p})$. 算法搜索理想类中的元素 $\mathfrak{b}\in \left[\mathfrak{a}\right]$ 使得 $\mathfrak{b}$ 在 $\mathcal{O}_{\Delta_p}$ 中与 $p$ <mark>互质</mark>. 搜索算法见于[HJPT98, Algorithm 1]. 进而,
$$
...
By [Fraleigh8ed, Theorem 12.7], we have $[1+\sqrt{k}]^m=[(1+\sqrt{k})^m]$. Next, we have
...
### Q5. 判别式 $K=-pq$中的参数 $p,q$ 的选择是否遵循什么道理?
...
文献[HM00]建议尽可能缩小类群的2-Sylow子群. 文献[Kap78, p. 598]还指出, 如果 $(p/q)=(q/p)=-1$, 那么 $C(K)$ 的2-Sylow子群的阶就只有2.

根据文献[Coh00, Theorem 4.9.15], 类数 $h(K)$ 的 "平均长度" 约为 $\sqrt{K}$. 根据[CL15, Appendix B.4], $g$ 的阶约为 $\sqrt{K}$.

### Q6. 在[CL15, Fig. 2]中, 什么叫 set $\mathfrak{r}$ an ideal lying above $r$?
...
没错, $f^m$ 离散对数的确是谁都能解. 这是因为 $f^m=[(p^2, m^{-1}p)]$, 明文就是对第二个参数求模p乘法逆元. <mark>但是 $c_2 = h^r f^m$ 就没这么简单了</mark>. 只有私钥持有者才能构造出 $c_1^{-x} = h^{-r}$, 从而消掉 $c_2$ 中的 $h_r$.
...
Step 2. 搜索小质数 $r\neq p$ 使得 $(\Delta_K/r)=1$. Kronecker符号已有实现.

```
https://docs.rs/rug/latest/rug/struct.Integer.html#method.kronecker
```

Step 3. 求解 $b_r$ 使得 $b_r^2\equiv \Delta_K \pmod{4r}$. 定义 $O_{\Delta_K}$ 的理想 $\mathfrak{r}=(r,b_r)$. 求解二次余子需要Tonelli-Shanks算法, 已有如下Python实现. 这一步依据文献[HJPT98, Subsection 3.1].

```
https://github.com/GiacomoPope/ClassGroups/blob/d3020c3b7f7af9b6588ce2176b8b7c6cfbb18602/classgroup_helper.py#L40
```

Step 4. 计算如下公式. 其中函数 $\varphi_p^{-1}$ 详见本文Q1; $k$ 是随机数 $1\sim p$.
$$
g=\left[ \varphi_p^{-1}(\mathfrak{r}^2)\right]^{p}f^k.
$$
Step 5. (可选) 估计

### Q11. A faster variant?

[CL15, Section 4.2]开头提到的 $\psi$ 怎么计算?
