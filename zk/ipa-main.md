...
$$
其中 $m_j ( i ) \in \{ 0, 1 \}$ 是数字 $i$ 的二进制表示中第 $j$ 位 ( $j = 1$ 对应最高位, 因为第一轮按前半 / 后半切, 切的正是最高位 ) . 这一步是 $O ( n )$ , 是 IPA 验证线性的全部来源.
...
p.s. 除了直觉观察, 还有一条路也能得到下述猜想. 如果见过 FFT 或 sumcheck 或 GKR 里任何一个, 下述猜想 (在证出来之前) 的置信度就会很高.

猜想: 存在标量 $C_j$ 使得 $\mathbf{b}^{(j)}[i]=C_j\zeta^{i}$ 对所有合法 $i$ 都成立, 且
$$
C_j=C_{j-1}\big(
1+x_{j}^{-1}\zeta^{2^{k-j}}
\big)
...
归纳基础, 即 $j=0$ 时, $\mathbf{b}^{(0)}[i]=\zeta^i$, 即 $C_0=1$.

归纳步. 设 $\mathbf{b}^{(j-1)}[i]=C_{j-1}\zeta^i$. 如此, $\mathbf{b}^{(j)}$ 的长度为 $m_j:=2^{k-j-1}$. 对 $i=0,\dots,m_j-1$, 有
$$
\begin{align}
\mathbf{b}^{(j)}[i]
&=\mathbf{b}^{(j-1)}[i]+x_j^{-1}\mathbf{b}^{(j-1)}[i+m_j] \\
&=C_{j-1}\zeta^{i}+x_j^{-1}C_{j-1}\zeta^{i+m_j} \\
&=C_{j-1}\zeta^{i}\,(1+x_j^{-1}\zeta^{m_j}).
\end{align}
$$
最后一步是 $j=k$. 注意到 $\mathbf{b}^{(k)}$ 只有一个元素, 故有
$$
\mathbf{b}^{(k)}
=\mathbf{b}^{(k)}[0]
=\prod_{j=1}^k\big(
    1+x_j^{-1}\zeta^{2^{k-j}}\big
).
$$
