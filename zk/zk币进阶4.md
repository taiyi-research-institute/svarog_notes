...
* spend 电路 (每张输入钞票一个): 成员、核销正确、花费授权, 外加一条新命题 —— $\mathtt{cv}$ 与承诺 $C$ 里藏的是**同一个** $v$.
* output 电路 (每张输出钞票一个): 产出承诺正确、范围 $0 \le v < 2^{64}$, 外加同一条新命题.
...
* 2 个 spend 证明: 各证一张输入钞票的成员、核销、授权、$\mathtt{cv}$ 一致.
