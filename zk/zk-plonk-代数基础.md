...
至此, 我们要证明: gx-pub 和 gx-pri 背后是同一个多项式.

根据 Schwartz-Zippel 引理, 对一条次数为 $d$ 的多项式恒等式
$$
G(X) - (X^n-1)\cdot t(X)=0,
$$
验证者在有限域 $\mathbb{GF}(p)$ 上取一点 $\zeta$, 只检查
$$
G(\zeta)-(\zeta^n-1)\cdot t(\zeta)\stackrel{?}{=}0.
$$
若恒等式不成立, 那么它在 $\zeta$ 上碰巧成立的概率至多为 $d/p$. 对于足够大的 $p$, 这个概率可忽略.

如此, 验证者只需索要 $a(\zeta)$, $b(\zeta)$, $c(\zeta)$, $t(\zeta)$; 且要确信这些值来自证明者承诺的那条多项式.

协议分四阶段. 有三轮通信, 可用 Fiat-Shamir 压缩到一轮.

(1) 承诺阶段, 也就是 $\zeta$ 出现之前.

证明者发送 `Com(a)`, `Com(b)`, `Com(c)`, `Com(t)`.

(2) 挑战阶段.

验证者发送随机点 $\zeta$. 或采用 Fiat-Shamir, 约定一个哈希顺序, 从当前聊天记录 (transcript) 导出 $\zeta$.

(3) 验证阶段.

(3.1) 验证各多项式真实存在.

证明者公布秘密多项式在 $\zeta$ 处的取值, 即 $a(\zeta)$, $b(\zeta)$, $c(\zeta)$, $t(\zeta)$; 以及每条多项式的证明 $\pi_a$, $\pi_b$, $\pi_c$, $\pi_t$. 验证者验证 $\pi_a$, $\pi_b$, $\pi_c$, $\pi_t$. 证明和验证的具体算法有 KZG, Halo 2 等.

(3.2) 验证 gx-pri 和 gx-pub 用的是同一个 $G(X)$.

验证者检查
$$
G(\zeta)
\stackrel{?}{=}
t(\zeta)\cdot(\zeta^n-1)\pmod{p}.
$$
协议至此结束.
...
上面的朴素协议只是骨架, 真实系统还要在它之上叠加 "连线约束", 也就是约束诸如 "第 1 行的 $c$ 必须等于第 2 行的 $a$" 这类跨格相等关系. 这些电路连线由另一条名为 "置换论证" 的多项式方程单独保证.
