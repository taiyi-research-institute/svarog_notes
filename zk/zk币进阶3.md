...
钞票本身不变, 仍是 $(v,\mathtt{pk},\rho,r)$, 承诺 $C$ 照旧. 变的是每张输出钞票登记时附带两样东西:
...
命题与进阶2 完全一致: 登记、核销、花费授权、守恒、范围、发行. 调用参数多带 $(\mathtt{epk}_1,\mathtt{ct}_1), (\mathtt{epk}_2,\mathtt{ct}_2)$ —— 它们不是公开输入, 只是随交易携带的数据.

(3) 合约校验并记账.

照旧验证 $\pi$、记核销号、登记新钞; 只是发布事件时把 $(C_1,\mathtt{epk}_1,\mathtt{ct}_1)$, $(C_2,\mathtt{epk}_2,\mathtt{ct}_2)$ 一起发出去.
...
存款与进阶2 相同: 明文金额 + 发行证明. 唯一的新动作是给新登记的钞票照本篇附 $(\mathtt{epk}, \mathtt{ct})$, 加密给收款地址 (通常是存款人自己) —— 换设备恢复钱包时才扫得回来.
