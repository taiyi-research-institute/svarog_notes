...
设中间的矩阵为 $M_f$, 则有 $\Delta(f)=-4\det(M_f)$.

对于整系数二元二次型 $f=(a,b,c)$, 当且仅当

$$
\Delta<0 \quad\mathtt{AND}\quad  a>0
$$

时, $f$ 为正定的 (positive definite).

## 2. 二次型的规约

### 2.1 正规等价 (proper equivalence)

定义集合 $\mathrm{SL}(2,\mathbb{Z})$ 为行列式为1的2阶整数方阵的集合. 根据定义可以推出:
```
集合元素有无穷多个. 集合关于矩阵乘法构成群.
```

令 $U=\begin{bmatrix}s&u\\t&v\end{bmatrix}$ 为 $\mathrm{SL}(2,\mathbb Z)$ 的一个元素. 定义坐标变换函数
$$
U(x,y)=(sx+ty,ux+vy),
$$
...
$$
fU(x,y)=(\det U)\cdot f(U(x,y)).
$$

因为 $\det U=1$, 所以 $fU$ 与 $f$ 处在同一个正规等价类 (proper equivalence class).

### 2.2 primitive / reducible

定义: 若二次型满足 $\gcd(a,b,c)=1$, 则称为primitive (本原的).
